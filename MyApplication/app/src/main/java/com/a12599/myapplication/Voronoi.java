...
public class Voronoi {
    ...
    private double maxlat, minlat, maxlng, minlng;
    private double sweepLoc;
    private final ArrayList<Point> sites;
    final ArrayList<VoronoiEdge> edgeList;
    ...
    public Voronoi(ArrayList<Point> sites,double maxlat,double minlat,double maxlng,double minlng) {
        this(sites, false, maxlat, minlat, maxlng, minlng);
    }

    public Voronoi(ArrayList<Point> sites, boolean animate, double maxlat,double minlat,double maxlng,double minlng) {
        ...
        for (Point site : sites) {
            if ((site.x > maxlng || site.x < minlng) || (site.y > maxlat || site.y < minlat))
                throw new RuntimeException(String.format(
                    "lat must be between %f and %f, lng must be between %f and %f", minlat, maxlat, minlng, maxlng ));
        ...
        this.sweepLoc = minlat - 5; // hack to draw negative infinite points
